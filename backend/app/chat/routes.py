...
@chat_router.post("/{thread_id}/resume")
async def resume_stream(thread_id: UUID, approved: bool, current_user: CurrentUserDep):
    return StreamingResponse(
        await chat_service.resume_chat_stream(thread_id, approved, current_user.id),
